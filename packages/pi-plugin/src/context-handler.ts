...
async function runPipeline(args: RunPipelineArgs): Promise<RunPipelineResult> {
	...
	const volatileContext = __gamebuddyReadAuthoredVolatileMaterialization(
		args.sessionId,
	);
	...
	if (args.injection) {
		...
		try {
			...
			const priorStableHash = publishedStableContextHashBySession.get(
				args.sessionId,
			);
			...
			publishGameOperationalGateMaterialization(args.sessionId, {
				...
				materializedCategoryCounts:
					injectionResult.materializedMemoryCategoryCounts,
			});
