...
import { createTagger, type Tagger } from "@magic-context/core/features/magic-context/tagger";
import {
...
import { TEXT_TAG_IDENTITY_MARKER, tagTranscript } from "@magic-context/core/shared/tag-transcript";
...
import { formatPiPressureForLog, resolvePiPressureSnapshot } from "./pi-pressure";
...
function isPiHardCacheExpired(lastResponseTime: number, ttlMs: number, now: number): boolean {
...
export const __test = {
    ...
    setMutationGateObserverForTests(fn: typeof mutationGateObserverForTests): () => void {
    ...
    setVariantChangeLogObserverForTests(fn: typeof variantChangeLogObserverForTests): () => void {
        variantChangeLogObserverForTests = fn;
        return () => {
            variantChangeLogObserverForTests = undefined;
        };
    },
    setLkgRecoveryLogObserverForTests(fn: typeof lkgRecoveryLogObserverForTests): () => void {
    ...
    for (const [messageId, currentSources] of currentSourcesByMessageId) {
        ...
        const vectorMatches =
            ...
            legacyRows.every((row, index) => {
                ...
                const stored = withoutPiLeadingTemporalMarker(sourceCache.get(row.tagId) ?? "");
...
export function piVariantChangeBustsProviderCache(providerID?: string, modelID?: string): boolean {
...
function buildPiAlignedEntryIds(entries: readonly unknown[]): (string | undefined)[] {
    ...
    for (let index = entries.length - 1; index >= 0; index -= 1) {
        ...
        firstKeptEntryId =
            typeof row.firstKeptEntryId === "string" ? row.firstKeptEntryId : undefined;
        break;
    }
    if (compactionIndex < 0) {
...
function readPiBranchEntriesForContext(
    ...
    try {
        ...
        if (cached && cachedAncestorIndex === cached.entries.length - 1) {
            ...
            if (
                suffix.some(
                    (entry) => (entry as { type?: unknown } | undefined)?.type === "compaction",
                )
            ) {
...
function loadPiHistorianStateSnapshot(
    ...
    return {
        detectedContextLimit:
            typeof row?.detected_context_limit === "number" && row.detected_context_limit > 0
                ? row.detected_context_limit
                : 0,
        needsEmergencyRecovery:
            typeof row?.needs_emergency_recovery === "number" && row.needs_emergency_recovery > 0,
...
export function registerPiContextHandler(
    ...
    const lkgCoordinator = createPiLkgCoordinator(
        ...
        ({ sessionId, elapsedMs, reusedPrefix }) =>
            logTransformElapsed(sessionId, "lkgCapture", elapsedMs, `reusedPrefix=${reusedPrefix}`),
    );
    ...
    registerPiGuardedContext(pi, async (event, ctx) => {
        ...
        let sessionMetaForPass: ReturnType<typeof getOrCreateSessionMeta> | undefined;
        ...
        try {
            ...
            let piUsage: ReturnType<NonNullable<ExtensionContext["getContextUsage"]>> | undefined;
            try {
            ...
            const rawMessageProvider = {
                ...
                readMessagePage: (afterOrdinal: number, limit: number, finalWatermark: number) =>
                    branchEntries !== null
                        ? convertEntriesToRawMessagePage(
                              branchEntries,
                              afterOrdinal,
            ...
            const lkgEntryIds = reconcilePiLkgEntryIds(strictEntryIds, alignedEntryIds);
            ...
            const lkgProviderKey =
                typeof lkgContextModel?.provider === "string" ? lkgContextModel.provider : null;
            ...
            const modelChanged =
                ...
                piModelRefToCanonical(previousModelKey) !== piModelRefToCanonical(currentModelKey);
            ...
            if (
                ...
                updateSessionMeta(options.db, sessionId, {
                    ...
                    lastInputTokens:
                        sessionMeta.lastInputTokens > baseWindowGeometry.derivation.absoluteWall
                ...
                if (usageInputTokens > baseWindowGeometry.derivation.absoluteWall) {
                    ...
                    usagePercentage = (usageInputTokens / baseWindowGeometry.usableSoft) * 100;
                }
            }
            ...
            const emergencyPercentage = ctx.model ? hardUsagePercentage : usagePercentage;
            const isEmergency =
                !options.compactionOff && emergencyPercentage >= EMERGENCY_BLOCK_PERCENTAGE;
            if (isEmergency) {
            ...
            const result = await runPipeline({
                ...
                sessionMeta,
                messages: event.messages,
                lkgEntryIds: event.messages.map((message) => lkgInputIdByRef.get(message)),
                ...
                reasoningClearing: {
                    clearReasoningAge:
                        options.heuristics?.clearReasoningAge ?? DEFAULT_CLEAR_REASONING_AGE,
            ...
            clearLastTransformErrorIfSet(options.db, sessionId, sessionMeta.lastTransformError);
            ...
            if (assertTailHygieneLastWriter && process.env.NODE_ENV !== "production") {
                assertTailHygieneLastWriter();
            }
            ...
            if (
                (isFailClosedBlockingError(err) || err instanceof EmergencyFailClosedError) &&
 ...
 */
export async function awaitInFlightHistorians(sessionId?: string): Promise<void> {
...
function hasEligiblePiCompartmentHistory(
    ...
    try {
        ...
        if (!boundarySnapshot) return rawEligibility.offset <= rawEligibility.rawMessageCount;
...
function maybeFireHistorian(args: {
    ...
    piUsage: ReturnType<NonNullable<ExtensionContext["getContextUsage"]>> | undefined;
    ...
    try {
        ...
        if (sessionMeta.lastContextPercentage > 0 && sessionMeta.lastInputTokens > 0) {
            usage = {
...
async function runPipeline(args: RunPipelineArgs): Promise<RunPipelineResult> {
    ...
    try {
        nativeRemovalInputs = getNativeReplayState(args.db, args.sessionId).toolInputs;
    } catch {
    ...
    const transcript = createPiTranscript(args.messages, args.sessionId, args.entryIds, {
        preserveReasoningToolArcs: args.reasoningClearing?.preserveReasoningToolArcs,
    ...
    const cachedPrefixBeforePreflight =
        piM0State && (foldDueDecision.value || softRefreshOpportunity) && !emergencyDropEligible
            ? prepareCachedM0M1PiReplay(piM0State, args.db, injectionPassSnapshot?.cachedRow)
            : undefined;
    let preFoldInjectionResult: PiInjectionResult | null = null;
    const persistedM0BeforeFold = injectionPassSnapshot?.sessionMeta ?? args.sessionMeta;
        ...
        try {
            ...
            publishedM1RefreshedThisPass =
                ...
                persistedM0BeforeFold.cachedM1Bytes?.toString("utf8") !==
                    getOrCreateSessionMeta(args.db, args.sessionId).cachedM1Bytes?.toString("utf8");
    ...
    const usesTokenProtection =
        args.protectedTokenTierOverrides !== undefined || args.protectedTokens !== undefined;
    ...
    const shouldApplyPendingOps =
        (baseShouldApplyPendingOps || deferredMaterialize) && publishedWorkDrainAllowed;
    ...
    if (shouldApplyPendingOps) {
        ...
        try {
            ...
            if (pendingOpsDidMutate) shouldRunHeuristics = args.heuristics !== undefined;
    ...
    const flushedDroppedTags = getDroppedTagsByNumbers(args.db, args.sessionId, targetTagNumbers);
    logTransformTiming(
        args.sessionId,
        "getDroppedTagsByNumbers",
        tGetTags,
    ...
    if (shouldRunHeuristics && args.heuristics) {
        try {
            ...
            if (routineCleanupApplied && !args.sessionMeta.isSubagent && executePressureEligible) {
                routinePressureAppliedBySession.set(args.sessionId, true);
            }
    ...
    const undecidedTagNumbers = activeTags
        .filter(
            (tag) =>
                !contentDecisions.has(encodePiContentDecision("reminder-strip", tag.messageId)),
        )
        .map((tag) => tag.tagNumber);
    ...
    for (const tag of activeTags) {
        ...
        if (!content) continue;
        const encodedDecision = encodePiContentDecision("reminder-strip", tag.messageId);
        ...
        if (
            ...
            freezePiContentDecision(args.db, args.sessionId, "reminder-strip", tag.messageId)
        ) {
        ...
        const legacyStripped = !frozen && stripTagPrefix(legacySource) === stripTagPrefix(stripped);
    ...
    if (
        emergencyDropEligible &&
        (pendingOpsDidMutate || heuristicOrReasoningDidMutate || autoReclaimDidMutate)
    ) {
    ...
    let injectionResult: PiInjectionResult | null = null;
    if (args.injection) {
        if (piM0State === undefined) {
            throw new Error("pi_m0_m1_state_unavailable_during_injection");
        }
        try {
            ...
            const stablePublicationChanged =
                (stableContext !== undefined ||
                    publishedStableContextHashBySession.has(args.sessionId)) &&
            ...
            const wireInjectionResult = injectM0M1PiForRun(
                ...
                isCacheBustingPass || (stablePublicationChanged && stableContext !== undefined),
                injectionPassSnapshot,
                true,
            );
            injectionResult = preFoldInjectionResult?.m0Materialized
                ? {
                      ...wireInjectionResult,
                      m0Materialized: true,
                      m0Reason: preFoldInjectionResult.m0Reason ?? wireInjectionResult.m0Reason,
            ...
            if (
                ...
                const firstRetainedMessage = args.messages[injectionResult.syntheticLeadingCount];
                const id =
                ...
                if (
                    ...
                    freezePiContentDecision(args.db, args.sessionId, "seam-temporal-strip", id)
                ) {
                    contentDecisions.add(encodePiContentDecision("seam-temporal-strip", id));
                }
            }
            ...
            for (const message of args.messages) {
                ...
                if (
                    id &&
                    contentDecisions.has(encodePiContentDecision("seam-temporal-strip", id))
                ) {
...
function parseStoredArray<T>(raw: unknown, isValue: (value: unknown) => value is T): T[] {
...
function isAutoSearchHintDecision(value: unknown): value is AutoSearchHintDecision {
    if (!value || typeof value !== "object") return false;
    const row = value as Record<string, unknown>;
...
function applyNoteNudges(args: {
    ...
    const noteAnchors =
        args.postTransformSnapshot?.noteAnchors ?? getNoteNudgeAnchors(db, sessionId);
    ...
    const deferredNoteText =
        args.postTransformSnapshot?.noteTriggerPending === false
            ? null
            : peekNoteNudgeText(db, sessionId, latestUserId, projectIdentity, noteReadStillVisible);
    if (deferredNoteText) {
        if (entryIds === null) {
            sessionLog(
                sessionId,
...
function buildPiMessageIdByIndex(
    ...
    for (let index = 0; index < messages.length; index += 1) {
        ...
        if (entryIdByRef) {
            ...
            const byRef =
                msg && typeof msg === "object" ? entryIdByRef.get(msg as object) : undefined;
...
export function clearContextHandlerSession(sessionId: string, db?: ContextDatabase): void {
