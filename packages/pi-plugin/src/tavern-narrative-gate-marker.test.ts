...
describe("Tavern provider-start observation", () => {
	...
	it("validates the exact session schema and rejects malformed callbacks", () => {
		expect(() =>
			registerTavernProviderStartObserver("session/1", () => undefined),
		).toThrow("invalid_tavern_provider_start_observer_session");
		expect(() =>
			registerTavernProviderStartObserver("", () => undefined),
		).toThrow("invalid_tavern_provider_start_observer_session");
		expect(() =>
			registerTavernProviderStartObserver("session_1", undefined as never),
		).toThrow("invalid_tavern_provider_start_observer_callback");
	});
	...
	it("classifies 2xx as success and every other status as error without payload retention", () => {
		...
		provider(200);
		provider(201);
		expect((received[0] as Record<string, unknown>).statusClass).toBe(
			"success",
		);
	...
	it("never fires for a foreign or malformed session and never reads other surfaces", () => {
		...
		if (!afterResponse || !beforeSwitch || !shutdown)
