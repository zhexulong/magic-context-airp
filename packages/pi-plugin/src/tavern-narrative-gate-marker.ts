...
type TavernMarkerBinding = Readonly<{
	kind: "tavern";
	sessionId: string;
	nonceSha256: string;
}>;
...
const providerStartObservers = new Map<
	string,
	TavernProviderStartObserverBinding
>();
...
export function validateTavernNarrativeGateMarkerConfig(
	...
	if (
		...
		!Object.keys(candidate).every(
			(key) => key === "sessionId" || key === "nonceSha256",
		)
	)
		throw new Error("invalid_tavern_marker_config");
	if (
		...
		throw new Error("invalid_tavern_marker_config");
	if (
		typeof candidate.nonceSha256 !== "string" ||
		!SHA256.test(candidate.nonceSha256)
	)
		throw new Error("invalid_tavern_marker_config");
...
export function validateGameOperationalGateMarkerConfig(
	...
	if (
		...
		!Object.keys(candidate).every(
			(key) =>
				key === "sessionId" || key === "nonceSha256" || key === "surface",
		)
	)
		throw new Error("invalid_game_operational_gate_marker_config");
	if (
...
export function registerTavernNarrativeGateMarker(
	...
	const binding: TavernMarkerBinding = Object.freeze({
		kind: "tavern",
		...config,
	});
...
export function registerTavernProviderStartObserver(
	...
	if (typeof onStart !== "function")
		throw new Error("invalid_tavern_provider_start_observer_callback");
	...
	return () => {
		if (providerStartObservers.get(sessionId) === binding)
			providerStartObservers.delete(sessionId);
	};
}
...
/** Test helper; never exposes a binding, callback, or observation payload. */
export function hasTavernProviderStartObserverForTest(
	sessionId: string,
): boolean {
	return providerStartObservers.has(sessionId);
}

function consumeProviderStartObserver(
	sessionId: string,
	statusClass: "success" | "error",
): void {
...
function reportProviderStartObservation(
	sessionId: string,
	status: number,
): void {
	const statusClass =
		status >= 200 && status < 300 ? ("success" as const) : ("error" as const);
	consumeProviderStartObserver(sessionId, statusClass);
}
...
export function registerTavernNarrativeGateMarkerHook(pi: ExtensionAPI): void {
	...
	pi.on("session_shutdown", (_event, ctx) => {
		...
		if (typeof sessionId === "string")
			clearTavernNarrativeGateMarker(sessionId);
	});
}
