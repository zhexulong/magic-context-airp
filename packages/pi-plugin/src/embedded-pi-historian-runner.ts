...
export class EmbeddedPiHistorianRunner implements SubagentRunner {
	...
	constructor(private readonly streamCall: typeof stream = stream) {}
	...
	async run(options: SubagentRunOptions): Promise<SubagentRunResult> {
		...
		if (options.signal?.aborted)
			return fail("abort", "embedded historian aborted by caller");
		if (!this.registry)
			return fail(
				"spawn_failed",
				"embedded historian ModelRegistry is not bound",
				true,
			);
		if (!options.model)
			return fail(
				"model_failed",
				"embedded historian requires a configured model",
			);
		...
		if (slash <= 0 || slash === options.model.length - 1) {
			return fail(
				"model_failed",
				"embedded historian model must be provider/model",
			);
		}
		...
		if (!model)
			return fail(
				"model_failed",
				`embedded historian model unavailable: ${options.model}`,
				true,
			);
		...
		try {
			const context: Context = {
				...
				messages: [
					{ role: "user", content: options.userMessage, timestamp: Date.now() },
				],
			...
			if (controller.signal.aborted) {
				return fail(
					options.signal?.aborted ? "abort" : "timeout",
					options.signal?.aborted
						? "embedded historian aborted by caller"
						: "embedded historian timed out",
					true,
				);
			}
			if (
				...
				return fail(
					"model_failed",
					final?.errorMessage ??
						"embedded historian returned no final assistant message",
					true,
				);
			}
			if (final.stopReason === "length")
				return fail(
					"truncated",
					"embedded historian output was truncated",
					true,
				);
