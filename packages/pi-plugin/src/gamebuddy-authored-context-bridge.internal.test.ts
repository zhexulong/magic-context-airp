...
const scope = {
	...
	threadId: "t",
	profile: { profileId: "p", revision: 1, canonicalHash: "a".repeat(64) },
};
...
const catalog = () => {
	...
	return {
		...body,
		canonicalHash: createHash("sha256").update(canonical(body)).digest("hex"),
	};
};
...
describe("authored context private bridge", () => {
	it("supersedes old capabilities without allowing them to clear the replacement", async () => {
		const oldCapability = publishGameBuddyAuthoredStableCatalog(
			scope,
			catalog(),
		);
		...
		const newCapability = publishGameBuddyAuthoredStableCatalog(
			scope,
			replacement,
		);
		...
		expect(() =>
			oldCapability.assertInstall("turn_old", oldPlan.sourceRefs),
		).toThrow("gamebuddy_authored_context_stale_superseded");
		expect(() =>
			newCapability.assertInstall("turn_new", newPlan.sourceRefs),
		).not.toThrow();
		await oldCapability.clear();
		expect(() =>
			newCapability.prepare("preflight_still_current"),
		).not.toThrow();
	...
	it("rejects a different thread or profile sharing the same Pi session", async () => {
		...
		const foreignCatalog = {
			...foreignBody,
			canonicalHash: createHash("sha256")
				.update(canonical(foreignBody))
				.digest("hex"),
		};
		expect(() =>
			publishGameBuddyAuthoredStableCatalog(foreignScope, foreignCatalog),
	...
	it("binds volatile refs to one durable turn, is idempotent, and rejects cross-turn use", async () => {
		...
		const volatileSource = {
			...
			canonicalHash: createHash("sha256").update(volatileContent).digest("hex"),
			budgetTokens: 7,
			totalOrderKey: "0002",
			provenance: "fixture",
		...
		const volatileCatalog = {
			...body,
			canonicalHash: createHash("sha256").update(canonical(body)).digest("hex"),
		};
		const capability = publishGameBuddyAuthoredStableCatalog(
			scope,
			volatileCatalog,
		);
		const first = capability.materializeVolatileForTurn(
			"turn_1",
			"Moon Festival",
			"tail",
		);
		const second = capability.materializeVolatileForTurn(
			"turn_1",
			"Moon Festival",
			"tail",
		);
		...
		expect(
			__gamebuddyReadAuthoredVolatileMaterialization(scope.sessionId)
				?.durableTurnId,
		).toBe("turn_1");
		expect(() =>
			capability.materializeVolatileForTurn("turn_2", "Moon Festival", "tail"),
		).toThrow("cross_turn");
	...
	it("returns reference-only plans and rejects mismatched refs", async () => {
		...
		expect(() =>
			capability.assertInstall("turn_01", [
				{ ...plan.sourceRefs[0], revision: "2" },
			]),
		).toThrow("gamebuddy_authored_context_plan_mismatch");
		await capability.clear();
		expect(() => capability.prepare("preflight_02")).toThrow(
			"gamebuddy_authored_context_cleared",
		);
		await capability.clear();
	});
});
