...
function mintCapability(scope: GameBuddyChatContextScope, materialization: ReturnType<typeof materializeGameBuddyAuthoredStableCatalog>): TavernAuthoredContextRuntimeCapability {
  ...
  capability = Object.freeze({
   ...
   materializeVolatileForTurn(turnId, acceptedPlayerText, boundedVisibleTail) { ids(turnId, "durable_turn_id"); assertCurrent(); const corpus = `${acceptedPlayerText}\n${boundedVisibleTail}`.normalize("NFC").toLocaleLowerCase(); const refs = entry.materialization.volatileSources.filter((source) => source.selectionKeys.some((key) => corpus.includes(key.normalize("NFC").trim().toLocaleLowerCase()))).map(({ content: _, budgetTokens: __, selectionKeys: ___, ...ref }) => Object.freeze(ref)); const result = Object.freeze({ refs: Object.freeze(refs), tokenCount: refs.reduce((total, ref) => total + (entry.materialization.volatileSources.find((source) => source.sourceId === ref.sourceId)?.budgetTokens ?? 0), 0) }); __gamebuddyBindAuthoredVolatileTurn(scope.sessionId, { turnId, sourceRefs: result.refs }); entry.volatileTurnId = turnId; return result; },
