...
export type TavernAuthoredContextRuntimeCapability = Readonly<{
    ...
    ): Readonly<{ refs: readonly GameBuddyAuthoredVolatileSourceRef[]; tokenCount: number }>;
    clear(): Promise<void>;
}>;
...
type CapabilityEntry = {
    ...
    readonly materialization: ReturnType<typeof materializeGameBuddyAuthoredStableCatalog>;
    active: boolean;
...
function mintCapability(
    ...
    const assertCurrent = (): void => {
        ...
        if (__gamebuddyReadAuthoredMaterialization(scope.sessionId) !== entry.materialization)
    ...
    const capability: TavernAuthoredContextRuntimeCapability = Object.freeze({
        ...
        assertInstall(turnId, refs, volatileRefs = []) {
            ...
            if (
                ...
                refs.some((r, i) => JSON.stringify(r) !== JSON.stringify(expected[i])) ||
        ...
        async clear() {
            ...
            if (entry.volatileTurnId !== undefined)
                __gamebuddyClearAuthoredVolatileTurn(scope.sessionId, entry.volatileTurnId);
            __gamebuddyClearAuthoredMaterialization(scope.sessionId, entry.materialization);
        },
    });
...
export function replaceGameBuddyAuthoredStableCatalog(
    ...
    if (
        ...
        __gamebuddyReadAuthoredMaterialization(scope.sessionId) !== entry.materialization
    )
