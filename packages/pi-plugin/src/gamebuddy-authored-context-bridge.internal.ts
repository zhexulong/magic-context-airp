...
type CapabilityEntry = { readonly scope: GameBuddyChatContextScope; readonly materialization: ReturnType<typeof materializeGameBuddyAuthoredStableCatalog>; active: boolean; volatileTurnId?: string };
...
function mintCapability(scope: GameBuddyChatContextScope, materialization: ReturnType<typeof materializeGameBuddyAuthoredStableCatalog>): TavernAuthoredContextRuntimeCapability {
  ...
  capability = Object.freeze({
    prepare(id) { ids(id, "transient_preflight_id"); assertCurrent(); return Object.freeze({ sourceRefs: Object.freeze(entry.materialization.sources.map(({ content: _, budgetTokens: __, provenance: ___, ...ref }) => Object.freeze(ref))), stableTokenCount: entry.materialization.budgetTokens, volatileSourceRefs: Object.freeze([]), volatileSourceCandidates: Object.freeze(entry.materialization.volatileSources.map((source) => Object.freeze({ ...source, selectionKeys: Object.freeze(source.selectionKeys) }))), volatileTokenCount: entry.materialization.volatileSources.reduce((total, source) => total + source.budgetTokens, 0) }); },
     assertInstall(turnId, refs, volatileRefs = []) { ids(turnId, "durable_turn_id"); assertCurrent(); const expected = entry.materialization.sources.map(({ content: _, budgetTokens: __, provenance: ___, ...ref }) => ref); const expectedVolatile = entry.materialization.volatileSources.map(({ content: _, budgetTokens: __, selectionKeys: ___, ...ref }) => ref); if (refs.length !== expected.length || refs.some((r, i) => JSON.stringify(r) !== JSON.stringify(expected[i])) || volatileRefs.length > expectedVolatile.length || volatileRefs.some((r, i) => JSON.stringify(r) !== JSON.stringify(expectedVolatile[i]))) throw new Error("gamebuddy_authored_context_plan_mismatch"); __gamebuddyBindAuthoredVolatileTurn(scope.sessionId, { turnId, sourceRefs: volatileRefs }); entry.volatileTurnId = turnId; },
     clearVolatileForTurn(turnId) { ids(turnId, "durable_turn_id"); if (entry.volatileTurnId === undefined) return; if (entry.volatileTurnId !== turnId) throw new Error("gamebuddy_authored_context_cross_turn"); __gamebuddyClearAuthoredVolatileTurn(scope.sessionId, turnId); entry.volatileTurnId = undefined; },
   materializeVolatileForTurn(turnId, acceptedPlayerText, boundedVisibleTail) { ids(turnId, "durable_turn_id"); assertCurrent(); const corpus = `${acceptedPlayerText}\n${boundedVisibleTail}`.normalize("NFC").toLocaleLowerCase(); const refs = entry.materialization.volatileSources.filter((source) => source.selectionKeys.some((key) => { const trimmed = key.normalize("NFC").trim().toLocaleLowerCase(); return trimmed.length > 0 && corpus.includes(trimmed); })).map(({ content: _, budgetTokens: __, selectionKeys: ___, ...ref }) => Object.freeze(ref)); const result = Object.freeze({ refs: Object.freeze(refs), tokenCount: refs.reduce((total, ref) => total + (entry.materialization.volatileSources.find((source) => source.sourceId === ref.sourceId)?.budgetTokens ?? 0), 0) }); __gamebuddyBindAuthoredVolatileTurn(scope.sessionId, { turnId, sourceRefs: result.refs }); entry.volatileTurnId = turnId; return result; },
     async clear() { if (!entry.active) return; entry.active = false; if (entry.volatileTurnId !== undefined) __gamebuddyClearAuthoredVolatileTurn(scope.sessionId, entry.volatileTurnId); __gamebuddyClearAuthoredMaterialization(scope.sessionId, entry.materialization); },
  });
...
export function publishGameBuddyAuthoredStableCatalog(scope: GameBuddyChatContextScope, catalog: unknown): TavernAuthoredContextRuntimeCapability {
  ...
  const materialization = materializeGameBuddyAuthoredStableCatalog(catalog, scope);
