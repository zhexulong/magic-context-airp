...
export type EmbeddedHistorianGateScenario =
	| "semantic"
	| "interaction"
	| "ordinary-process";
...
export async function runEmbeddedHistorianAuthoringGateForTest(
	...
	if (!acquireCompartmentLease(input.db, sessionId, holderId)) {
		throw new Error(
			"embedded Historian gate could not acquire its temporary lease",
		);
	}
	...
	const memories = getMemoriesByProject(
		input.db,
		resolveProjectIdentity(input.directory),
	);
	...
	const telemetry = input.db
		...
		| {
				status?: unknown;
				facts_emitted?: unknown;
				facts_by_category_json?: unknown;
		  }
		| undefined;
	const factsByCategory = (() => {
		try {
			const parsed: unknown = JSON.parse(
				typeof telemetry?.facts_by_category_json === "string"
					? telemetry.facts_by_category_json
					: "{}",
			);
			return parsed !== null &&
	...
	const count = (category: string) =>
		typeof factsByCategory[category] === "number" &&
		Number.isSafeInteger(factsByCategory[category])
	...
	return {
		...
		factsEmitted:
			typeof telemetry.facts_emitted === "number" ? telemetry.facts_emitted : 0,
